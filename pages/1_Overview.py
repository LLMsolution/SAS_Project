...
if len(filtered_df) > 0:
    ...
    with col1:
        st.markdown("#### Material Consumption Statistics")

        if len(consumption_data) > 0:
            st.markdown(f"**C-Checks with consumption data:** {len(consumption_data)} / {len(filtered_df)} ({len(consumption_data)/len(filtered_df)*100:.1f}%)")
            ...
            st.metric("Total Parts Consumed", f"{total_parts:,.0f}")

            parts_stats_col1, parts_stats_col2 = st.columns(2)
            with parts_stats_col1:
                st.metric("Average Parts/C-Check", f"{avg_parts:.0f}")
            ...
            with cost_stats_col1:
                st.metric("Average Cost/C-Check", f"€{avg_cost:,.0f}")
            with cost_stats_col2:
                st.metric("Median Cost/C-Check", f"€{median_cost:,.0f}")
        else:
            st.info("No consumption data available for selected filters")

    with col2:
        st.markdown("#### Planned Material Statistics")

        planned_data = filtered_df[filtered_df['planned_parts_count'].notna()].copy()

        if len(planned_data) > 0:
            st.markdown(f"**C-Checks with planned data:** {len(planned_data)} / {len(filtered_df)} ({len(planned_data)/len(filtered_df)*100:.1f}%)")
            ...
            st.metric("Total Parts Planned", f"{total_planned_parts:,.0f}")

            planned_stats_col1, planned_stats_col2 = st.columns(2)
            with planned_stats_col1:
                st.metric("Average Parts/C-Check", f"{avg_planned_parts:.0f}")
            ...
            st.metric("Total Planned Cost", f"€{total_planned_cost:,.0f}")

            planned_cost_col1, planned_cost_col2 = st.columns(2)
            with planned_cost_col1:
                st.metric("Average Cost/C-Check", f"€{avg_planned_cost:,.0f}")
            with planned_cost_col2:
                st.metric("Median Cost/C-Check", f"€{median_planned_cost:,.0f}")
        else:
    ...
    if len(variance_data) > 0:
        ...
        with col2:
            ...
            st.metric("Avg Cost Variance", f"€{avg_cost_variance:+,.0f}")
